...
describe("StormGuard", () => {
  ...
  it("logs a [chbus] warning when a message is dropped", () => {
    ...
    const guard = new StormGuard("my-channel", {
      maxMessages: 1,
      windowMs: 1000,
    });
    guard.check("svc");
    ...
    expect(warn).toHaveBeenCalledWith(expect.stringContaining("my-channel"));
    expect(warn).toHaveBeenCalledWith(expect.stringContaining("svc"));
  });
